...
Despliega Uniro en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.uniro` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/uniro
sudo chown $USER:$USER /var/lib/uniro
```
...
Crea `/etc/nginx/sites-available/uniro`:
...
sudo ln -s /etc/nginx/sites-available/uniro /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de Uniro
...
tar -czf uniro-backup-$(date +%Y%m%d).tar.gz /var/lib/uniro

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/uniro-$(date +\%Y\%m\%d).tar.gz /var/lib/uniro
```
...
```bash
# Verificar si Uniro está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/uniro
chmod 755 /var/lib/uniro
```
