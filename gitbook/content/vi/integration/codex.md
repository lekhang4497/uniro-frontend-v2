...
Tích hợp Uniro với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của Uniro.
...
Uniro cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong Uniro dashboard
...
1. Xác minh tên model khớp với cấu hình Uniro
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong Uniro dashboard
...
Đảm bảo bạn đã cấu hình API key trong Uniro cloud dashboard.
