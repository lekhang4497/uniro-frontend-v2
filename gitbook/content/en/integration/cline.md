...
Integrate Uniro with Cline VSCode extension to route your AI requests through Uniro's intelligent routing system.

## Prerequisites

- Visual Studio Code installed
- Cline extension installed from VSCode marketplace
- Uniro running locally or cloud endpoint configured
...
Set the base URL to your Uniro endpoint:
...
**Steps:**
1. In the **Base URL** field, enter your Uniro endpoint
...
3. The key should start with `sk-uniro-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your Uniro configuration
...
4. Cline will use Uniro to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using Uniro
...
3. Get AI-powered explanations through Uniro

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use Uniro to understand context and make changes
...
1. Verify Uniro is running: `curl http://localhost:20128/health`
...
2. Verify your Uniro instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check Uniro logs for any errors

## Advanced Configuration

### Using Cloud Endpoint

To use Uniro cloud endpoint instead of localhost:

1. In Cline settings, set Base URL to: `https://www.uniro.tech`
2. Make sure you have configured your API key in the Uniro cloud dashboard
...
## Integration with Uniro Features

### Model Routing

Uniro automatically routes your requests to the best available provider based on:
- Model availability
...
If a provider fails, Uniro automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through Uniro dashboard:
- Total requests
- Token usage
- Cost per model
