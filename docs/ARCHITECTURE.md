...
Uniro is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[Uniro Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.uniro/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.uniro/usage.json`, `~/.uniro/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.uniro/db.json`)
- usage stats: `~/.uniro/usage.json`
- request log lines: `~/.uniro/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Uniro Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.uniro` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/uniro && npm run build`
- Build Docker image: `cd /root/dev/uniro && docker build -t uniro .`
