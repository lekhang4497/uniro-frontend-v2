  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "uniro")
        : path.join(os.homedir(), ".uniro"),
      "mitm",
      ".mitm.pid"
    );
...
// Collect PIDs of all uniro-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from uniro install dir or wrapper cli.js
        const isAppProcess = lower.includes("uniro") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("uniro") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "uniro");
  }
  return path.join(os.homedir(), ".uniro");
}
...
// Resolve npx/uniro binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx uniro` — works regardless of global bin path changes after npm i -g
