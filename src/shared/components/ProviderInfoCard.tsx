      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2">
        {rows.map((r) => (
          <div key={r.key} className="flex items-center gap-3 min-w-0">
            <span className="text-xs text-[var(--text-secondary)] w-28 shrink-0">{r.label}</span>
            {r.isLink ? (
              <a
                href={typeof r.raw === "string" ? r.raw : undefined}
                target="_blank"
                rel="noopener noreferrer"
                className={`text-sm text-[var(--accent-blue)] hover:underline truncate ${r.mono ? "font-mono" : ""}`}
              >
                {r.value}
              </a>
            ) : (
              <span className={`text-sm text-[var(--text-primary)] truncate ${r.mono ? "font-mono" : ""}`}>
                {r.value}
              </span>
            )}
          </div>
        ...
        {noticeText && (
          <div className="flex items-start gap-3 min-w-0 sm:col-span-2">
            <span className="text-xs text-[var(--text-secondary)] w-28 shrink-0 mt-0.5">Notice</span>
            <span className="text-sm text-[var(--text-primary)] leading-relaxed">{noticeText}</span>
          </div>
        )}
      </div>
    </Card>
