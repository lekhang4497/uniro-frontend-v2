...
export interface AddCustomEmbeddingModalProps {
  ...
  onSaved?: (node: ProviderNode) => void;
...
export default function AddCustomEmbeddingModal({
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const url = isEdit && node?.id ? `/api/provider-nodes/${node.id}` : "/api/provider-nodes";
      ...
      const data = (await res.json()) as { node?: ProviderNode };
      if (res.ok && data.node) {
    ...
    if (valid) {
      return (
        <>
          ...
          {dimensions && <span className="text-sm text-[var(--text-secondary)]">{dimensions} dims</span>}
        </>
      );
    }
    return (
