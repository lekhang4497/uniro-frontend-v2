...
const hasUniroConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Uniro");
};

const getUniroEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Uniro") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getUniroEntry(config);
...
// POST - Apply Uniro config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Uniro entry or append
    const idx = config.findIndex((e) => e.name === "Uniro");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Uniro entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Uniro");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Uniro removed from Copilot config",
    });
  } catch (error) {
