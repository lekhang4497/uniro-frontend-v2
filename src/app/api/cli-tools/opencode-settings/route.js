...
const hasUniroConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["uniro"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["uniro"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasUniro: hasUniroConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("uniro/") ? config.model.replace(/^uniro\//, "") : null,
...
// POST - Apply Uniro as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_uniro";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing uniro provider entry and its models
    const existingProvider = config.provider["uniro"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["uniro"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `uniro/${finalActive}`;
      }
    }
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("uniro/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Uniro provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["uniro"]?.models) {
      delete config.provider["uniro"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["uniro"].models).length === 0) {
        delete config.provider["uniro"];
        if (config.model?.startsWith("uniro/")) delete config.model;
      } else if (config.model === `uniro/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["uniro"].models);
        config.model = `uniro/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire uniro provider
      if (config.provider) delete config.provider["uniro"];
      if (config.model?.startsWith("uniro/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("uniro/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Uniro settings removed from OpenCode",
    });
  } catch (error) {
