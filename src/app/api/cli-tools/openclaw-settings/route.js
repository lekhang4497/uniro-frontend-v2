...
// Check if settings has Uniro config
const hasUniroConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["uniro"];
};

// Read per-agent models.json and return current model id (without "uniro/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["uniro"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasUniro: hasUniroConfig(settings),
...
// POST - Update Uniro settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `uniro/${model}`;

    // Remove all old uniro/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("uniro/"))
    ...
    // Add fresh uniro models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`uniro/${m}`] = {};
    });

    // Remove old uniro model from each agent in agents.list
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (agent.model?.startsWith("uniro/")) {
    ...
    // Update models.providers.uniro with all models
    settings.models.providers["uniro"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `uniro/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Uniro settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Uniro from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["uniro"];
    ...
    // Remove uniro models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("uniro/"));
    ...
    // Reset agents.defaults.model.primary if it uses uniro
    if (settings.agents?.defaults?.model?.primary?.startsWith("uniro/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "Uniro settings removed successfully",
    });
  } catch (error) {
