...
import { ArrowRight, Check, ChevronDown, Copy, Eye, EyeOff, Loader2, Save, TriangleAlert, Undo2, X } from "lucide-react";
...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }: any) {
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_uniro for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# Uniro Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "uniro"

[model_providers.uniro]
name = "Uniro"
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-[var(--text-secondary)]">Manual configuration is still available if uniro is deployed on a remote server.</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 pl-9">
                  ...
                  <Button variant="outline" size="sm" onClick={() => setShowInstallGuide(!showInstallGuide)}>
                    <DynIcon name={showInstallGuide ? "expand_less" : "help"} size={18} className="mr-1" />
                    {showInstallGuide ? "Hide" : "How to Install"}
                  </Button>
                </div>
              </div>
              {showInstallGuide && (
                <div className="p-4 bg-[var(--bg-elevated)] border border-border rounded-lg">
                  <h4 className="font-medium mb-3">Installation Guide</h4>
                  <div className="space-y-3 text-sm">
                    <div>
                      <p className="text-[var(--text-secondary)] mb-1">macOS / Linux / Windows:</p>
                      <code className="block px-3 py-2 bg-black/5 dark:bg-white/5 rounded font-mono text-xs">npm install -g @openai/codex</code>
                    </div>
                    <p className="text-[var(--text-secondary)]">After installation, run <code className="px-1 bg-black/5 dark:bg-white/5 rounded">codex</code> to verify.</p>
          ...
          {!checkingCodex && codexStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {codexStatus?.config && (() => {
                  ...
                  return currentBaseUrl ? (
                    <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                      <span className="text-xs font-semibold text-[var(--text-primary)] sm:text-right sm:text-sm">Current</span>
                      <ArrowRight size={14} className="hidden text-[var(--text-secondary)] sm:inline" />
                      <span className="min-w-0 truncate rounded bg-[var(--bg-elevated)]/40 px-2 py-2 text-xs text-[var(--text-secondary)] sm:py-1.5">
                        {currentBaseUrl}
                      </span>
                    </div>
                  ) : null;
                ...
                {/* Model */}
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  <span className="text-xs font-semibold text-[var(--text-primary)] sm:text-right sm:text-sm">Model</span>
                  <ArrowRight size={14} className="hidden text-[var(--text-secondary)] sm:inline" />
                  <div className="relative w-full min-w-0">
                    <input type="text" value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} placeholder="provider/model-id" className="w-full min-w-0 pl-2 pr-7 py-2 bg-[var(--bg-elevated)] rounded border border-border text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5" />
                    {selectedModel && <button onClick={() => setSelectedModel("")} className="absolute right-1 top-1/2 -translate-y-1/2 p-0.5 text-[var(--text-secondary)] hover:text-red-500 rounded transition-colors" title="Clear"><X size={14} /></button>}
                  </div>
                  <button onClick={() => setModalOpen(true)} disabled={!activeProviders?.length} className={`w-full sm:w-auto rounded border px-2 py-2 text-xs transition-colors sm:py-1.5 whitespace-nowrap sm:shrink-0 ${activeProviders?.length ? "bg-[var(--bg-elevated)] border-border text-[var(--text-primary)] hover:border-primary cursor-pointer" : "opacity-50 cursor-not-allowed border-border"}`}>Select Model</button>
                </div>

                {/* Subagent Model */}
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  <span className="text-xs font-semibold text-[var(--text-primary)] sm:text-right sm:text-sm">Subagent Model</span>
                  <ArrowRight size={14} className="hidden text-[var(--text-secondary)] sm:inline" />
                  <div className="relative w-full min-w-0">
                    <input
                      type="text"
                      ...
                      className="w-full min-w-0 pl-2 pr-7 py-2 bg-[var(--bg-elevated)] rounded border border-border text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5"
                    />
                    {subagentModel && (
                      <button
                  ...
                  <button
                    ...
                    className={`w-full sm:w-auto rounded border px-2 py-2 text-xs transition-colors sm:py-1.5 whitespace-nowrap sm:shrink-0 ${activeProviders?.length ? "bg-[var(--bg-elevated)] border-border text-[var(--text-primary)] hover:border-primary cursor-pointer" : "opacity-50 cursor-not-allowed border-border"}`}
                  >
                    Select Model
                  </button>
                </div>
