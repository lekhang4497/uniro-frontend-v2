...
import { ArrowRight, Check, ChevronDown, Copy, Eye, EyeOff, Info, Loader2, Save, TriangleAlert, Undo2, X } from "lucide-react";
...
export default function ClaudeToolCard({
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      // Get key from dropdown, fallback to first key or sk_uniro for localhost
              ...
              {showInstallGuide && (
                <div className="p-4 bg-[var(--bg-elevated)] border border-border rounded-lg">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <p className="text-[var(--text-secondary)]">After installation, run <code className="px-1 bg-black/5 dark:bg-white/5 rounded">claude</code> to verify.</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {!checkingClaude && claudeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {tool.defaultModels.map((model) => (
                  <div key={model.alias} className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    <span className="text-xs font-semibold text-[var(--text-primary)] sm:text-right sm:text-sm">{model.name}</span>
                    <ArrowRight size={14} className="hidden text-[var(--text-secondary)] sm:inline" />
                    <div className="relative w-full min-w-0">
                      <input type="text" value={modelMappings[model.alias] || ""} onChange={(e) => onModelMappingChange(model.alias, e.target.value)} placeholder="provider/model-id" className="w-full min-w-0 pl-2 pr-7 py-2 bg-[var(--bg-elevated)] rounded border border-border text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5" />
                      {modelMappings[model.alias] && <button onClick={() => onModelMappingChange(model.alias, "")} className="absolute right-1 top-1/2 -translate-y-1/2 p-0.5 text-[var(--text-secondary)] hover:text-red-500 rounded transition-colors" title="Clear"><X size={14} /></button>}
                    </div>
                    <button onClick={() => openModelSelector(model.alias)} disabled={!hasActiveProviders} className={`w-full sm:w-auto rounded border px-2 py-2 text-xs transition-colors sm:py-1.5 whitespace-nowrap sm:shrink-0 ${hasActiveProviders ? "bg-[var(--bg-elevated)] border-border text-[var(--text-primary)] hover:border-primary cursor-pointer" : "opacity-50 cursor-not-allowed border-border"}`}>Select Model</button>
                  </div>
                ))}

                {/* CC Filter Naming */}
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  <span className="text-xs font-semibold text-[var(--text-primary)] sm:text-right sm:text-sm">Filter naming</span>
                  <ArrowRight size={14} className="hidden text-[var(--text-secondary)] sm:inline" />
                  <label className="flex items-center gap-1.5 cursor-pointer select-none">
                    ...
                    <span className="text-xs text-[var(--text-secondary)]">Filter naming requests</span>
                  </label>
                  <Tooltip text="Intercepts Claude Code's topic-naming requests and returns a fake response locally, saving API tokens.">
                    <Info size={14} className="text-[var(--text-secondary)] cursor-help" />
                  </Tooltip>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!claudeStatus?.hasUniro} loading={restoring}>
